...
public interface IMeetRepository extends IGenericRepository<Long, MeetJpa> {
    ...
    @Query(value = "select * from meets m where m.updatable_event_id =?1", nativeQuery = true)
    Optional<MeetJpa> findByUpdatableTargetId(Long id);

    @Modifying
    @Query(value = "delete from meets m where m.updatable_event_id = ?1", nativeQuery = true)
